...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, triplere_u,
        ...
        if model_name == 'ComplEx':
            # ComplEx is an inner-product model and needs a larger normal initialization to avoid vanishing gradients.
        ...
        # ComplEx represents both entities and relations as complex vectors, so both embeddings must be doubled.
    ...
    def TripleRE(self, head, relation, tail, mode):
        ...
        """
        # Split the relation vector into r_h, r_t, and r_m.
        r_h, r_t, r_m = torch.chunk(relation, 3, dim=2)

        # TripleREv2 formula: h * (r_h + u) - t * (r_t + u) + r_m.
        # self.u is passed from the command-line argument -3u.
    ...
    def CogRE(self, head, relation, tail, mode):
        ...
        # Keep this consistent with evaluation: tail projection minus head-relation projection.
    ...
    def ComplEx(self, head, relation, tail, mode):
        # Split the real and imaginary parts along the feature dimension.
        re_head, im_head = torch.chunk(head, 2, dim=2)
        re_relation, im_relation = torch.chunk(relation, 2, dim=2)
        re_tail, im_tail = torch.chunk(tail, 2, dim=2)

        # Compute the inner product and rely on broadcasting for negative samples.
        if mode == 'head-batch':
            re_score = re_relation * re_tail + im_relation * im_tail
            im_score = re_relation * im_tail - im_relation * re_tail
            score = re_head * re_score + im_head * im_score
        else: # single mode or tail-batch mode
            re_score = re_head * re_relation - im_head * im_relation
            im_score = re_head * im_relation + im_head * re_relation
