...
EMBEDDING_PATH = Path("./models/algebra2005/cog_h5_g09")
TEST_TRIPLES_PATH = DATA_PATH / "akt_fpkc_h5_g09_test_triples.txt"
OFFLINE_STATS_PATH = DATA_PATH / "raw_stats_akt.pkl"
STUDENT_HISTORY_PATH = Path("../pyKT_example/output_final_akt_fpkc_h5_g09.csv")
...
def parse_probability_token(token, prefix, default=0.0):
    ...
    for uid in uid_mlkc_dict.keys():
        ...
        if concept_keys:
            ...
            dist_mlkc = calc_alpha_l1(
                all_exercise_projection.unsqueeze(0) - hr_mlkc.unsqueeze(1)
            )

            dist_pkc = calc_alpha_l1(
                all_exercise_projection.unsqueeze(0) - hr_pkc.unsqueeze(1)
            )
        ...
        dist_efr = calc_alpha_l1(all_exercise_projection - state_efr_rec)
        fr2 = GAMMA - dist_efr
        ...
        for exercise_id in top_exercise_ids:
            ...
            for concept_id in concept_ids:
                ...
                if concept_key in user_mlkc:
                    prob = parse_probability_token(
                        user_mlkc[concept_key],
                        "mlkc",
                        default=global_history_mean,
                    )
                else:
...
def emcc_score_zpd(uid_pkc_dict, uid_ex_scores, q_matrix, n, student_kc_acc,
    ...
    Raw EMCC is the total expected learning gain over the concepts covered by
    the top-N recommendation list. It is not constrained to [0, 1].

    When normalize=True, the raw score is divided only by the PKC mass of the
    concepts covered by the top-N recommendation list:

        local_emcc = raw_expected_coverage / covered_need

    This evaluates the quality of the recommendation list on the concepts it
    actually covers, instead of dividing by all PKC values of the student.
    """
    emcc_values = []
    gamma_scale = 2.0
    ...
    for uid, scores in uid_ex_scores:
        ...
        for concept_id, alphas in concept_alphas.items():
            concept_key = f"kc{concept_id}"
            pkc_value = parse_probability_token(
                user_pkc.get(concept_key),
                "pkc",
                default=0.0,
            )
